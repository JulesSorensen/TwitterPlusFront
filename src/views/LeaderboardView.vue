<template>
  ...
  <div class="flex flex-col lg:flex-row lg:justify-around w-screen h-screen lg:px-10 dark:bg-gray-900">
    ...
    <div v-else class="flex flex-col w-full h-full lg:ml-5 mt-16 lg:mt-0">
      <h1 class="my-5 mx-auto font-bold text-xl dark:text-gray-300">Classement par nombre d'abonnées</h1>
      <div v-for="curAcc in accounts" :key="curAcc.id" v-on:click="openUser(curAcc.name)"
        class="flex flex-col lg:flex-row overflow-auto items-center lg:mb-3">
        <div class="h-10 w-10">
        ...
        <div class="flex flex-row">
          ...
          <p class="mx-1 dark:text-gray-200 hidden lg:block">·</p>
          <p class="text-sm w-[100px] min-w-[100px] dark:text-gray-200 hidden lg:block">{{ getTS(curAcc.createdAt) }}</p>
          <div v-if="curAcc.certification" class="w-5 h-5 ml-3 lg:ml-1">
            <img src="../assets/img/certif.png" alt="CERTIFIE">
          </div>
        </div>
        <p class="mx-1 dark:text-gray-200 hidden lg:block">·</p>
        <p class="mx-1 dark:text-gray-200 mb-10 lg:mb-0">{{ curAcc.subscribersNb }} abonnées</p>
      </div>
    </div>
  </div>
</template>
